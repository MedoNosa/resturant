...
export default function Price({ price, id, option }: Props) {
  ...
  useEffect(() => {
    setTotal(
      quantity * (option ? price + option[selected].additionalPrice : price)
    );
  }, [quantity, selected, option, price]);

  const handleAddToCart = () => {
    // هنا ممكن تضيف فعليًا للسلة لو عندك context أو redux
    toast.success("Success To Added😊");
  }
    ...
    <div className='flex flex-col gap-4'>
      ...
      <div className='flex gap-4'>
        {option?.map((opt, index) => (
          <button
            key={opt.title}
            className='min-w-[6rem] p-2 ring-1 ring-red-500 rounded-b-md cursor-pointer'
            style={{
      ...
      <div className='flex justify-between items-center'>
        ...
        <button
          className='uppercase w-65 bg-red-500 text-white p-3 ring-1 ring-red-500 rounded-b-md cursor-pointer'
